...
> **当前基准版本：v0.6.15**（2026-06-15 更新，已部署）

## ✅ v0.6.15 升级记录（2026-06-15 完成）

v0.6.15 相比 v0.6.13 **大幅简化**——上游 `rename to hermes-studio` 重构后，AppSidebar 的 UI 清理项（中转站、退出、版本检查、版本信息）已全部移除。

实际应用的改动：
- Web 品牌名 `Hermes Studio` → `NewHermes`（index.html、DesktopTitleBar.vue）
- 桌面端品牌 → `NewHermes Studio`（6 个源码文件 + artifactName）
- 移除 `DefaultCredentialPrompt`（App.vue）
- `login-limiter.ts` IP_MAX_FAILURES 改环境变量
- Logo/favicon 从源图重新生成

**v0.6.15 待处理项（非阻塞 Web 部署）：**
- `cli-shim.ts` 的 "Hermes Studio" echo/注释（混 CLI 命令名 `hermes-studio`，桌面发布时细处理）
- 桌面端图标 icon.png/ico/icns（桌面发布时处理）

## ⚠️ 线上验证方式（重要！）

**必须用 NodePort IP 直连**，不要用 localhost + Host header：

```bash
# ✅ 正确（直连 NodePort IP）
curl http://172.32.153.184:40080/ | grep -o '<title>[^<]*</title>'

# ❌ 错误（会误判 503，曾导致错误回滚）
curl -H "Host: agent1.172-32-153-184.nip.io" http://localhost:40080/
curl http://<pod-ip>:6060/
```

`localhost` 不是 184 节点的 NodePort，ingress 对该路径返回 503。pod IP / service ClusterIP 也走错网络路径会误报 503。**判断服务健康只信 `172.32.153.184:40080` 的结果**。
