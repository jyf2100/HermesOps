#!/usr/bin/env python3
"""
Skills Hub Core — Source adapters, data models, and search logic.

This module has NO side effects at import time. It does NOT call
``get_hermes_home()`` or reference any filesystem paths at module level.
All functions/classes that need a filesystem path accept it as a parameter.

Safe to import from admin backend and other contexts where HERMES_HOME
is not available.

Used by:
  - tools/skills_hub_local.py (agent-only hub operations)
  - admin backend (search/fetch without side effects)
  - hermes_cli/skills_hub.py (CLI commands and /skills slash command)
"""
...
def _read_index_cache(key: str, cache_dir: Optional[Path] = None) -> Optional[Any]:
    """Read cached data if not expired.

    Args:
        key: Cache key (filename without extension).
        cache_dir: Directory for cache files. If None, uses a temp-based default.
    """
...
def _write_index_cache(key: str, data: Any, cache_dir: Optional[Path] = None, hub_dir: Optional[Path] = None) -> None:
    """Write data to cache.
    ...
    Args:
        ...
        data: Data to cache.
        cache_dir: Directory for cache files. If None, uses a temp-based default.
        hub_dir: Parent hub directory for writing .ignore. If None, skipped.
    """
    if cache_dir is None:
        cache_dir = _default_cache_dir()
...
def _default_cache_dir() -> Path:
    """Return a default cache directory when no hermes home is available."""
    import tempfile
    return Path(tempfile.gettempdir()) / "hermes-skills-cache"
...
# ---------------------------------------------------------------------------
# Taps management (parameterized, no module-level path)
# ---------------------------------------------------------------------------
...
class TapsManager:
    ...
    def __init__(self, path: Optional[Path] = None):
        """
        Args:
            path: Path to taps.json. If None, uses default location.
        """
        if path is None:
            path = _default_cache_dir().parent / "taps.json"
        self.path = path
...
# ---------------------------------------------------------------------------
# Lock file management (parameterized, no module-level path)
# ---------------------------------------------------------------------------

class HubLockFile:
    """Manages the lock.json file -- tracks provenance of installed hub skills."""

    def __init__(self, path: Optional[Path] = None):
        """
        Args:
            path: Path to lock.json. If None, uses default location.
        """
        if path is None:
            path = _default_cache_dir().parent / "lock.json"
        self.path = path
...
class GitHubSource(SkillSource):
    ...
    def _read_cache(self, key: str) -> Optional[list]:
        ...
        cdir = self._cache_dir or _default_cache_dir()
...
class WellKnownSkillSource(SkillSource):
    ...
    def _parse_index(self, index_url: str) -> Optional[dict]:
        ...
        cached = _read_index_cache(cache_key, cache_dir=self._cache_dir)
        ...
        _write_index_cache(cache_key, parsed, cache_dir=self._cache_dir)
        return parsed
...
class SkillsShSource(SkillSource):
    ...
    def __init__(self, auth: GitHubAuth, cache_dir: Optional[Path] = None):
        self.auth = auth
        self._cache_dir = cache_dir
        self.github = GitHubSource(auth=auth, cache_dir=cache_dir)
    ...
    def search(self, query: str, limit: int = 10) -> List[SkillMeta]:
        ...
        _write_index_cache(cache_key, [_skill_meta_to_dict(item) for item in results], cache_dir=self._cache_dir)
        return results
    ...
    def _fetch_detail_page(self, identifier: str) -> Optional[dict]:
        ...
        if detail:
            _write_index_cache(cache_key, detail, cache_dir=self._cache_dir)
        return detail
...
class ClawHubSource(SkillSource):
    ...
    def search(self, query: str, limit: int = 10) -> List[SkillMeta]:
        ...
        _write_index_cache(cache_key, [_skill_meta_to_dict(s) for s in final_results], cache_dir=self._cache_dir)
    ...
    def _search_catalog(self, query: str, limit: int = 10) -> List[SkillMeta]:
        ...
        _write_index_cache(cache_key, [_skill_meta_to_dict(s) for s in results], cache_dir=self._cache_dir)
        return results
...
class ClaudeMarketplaceSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        gh = GitHubSource(auth=self.auth, cache_dir=self._cache_dir)
    ...
    def _fetch_marketplace_index(self, repo: str) -> List[dict]:
        ...
        _write_index_cache(cache_key, plugins, cache_dir=self._cache_dir)
        return plugins
...
class LobeHubSource(SkillSource):
    ...
    def _fetch_index(self) -> Optional[Any]:
        ...
        _write_index_cache(cache_key, data, cache_dir=self._cache_dir)
        return data
...
def _load_hermes_index(cache_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if cache_dir is None:
        cache_dir = _default_cache_dir()
    cache_file = cache_dir / "hermes-index.json"
    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            return _load_stale_index_cache(cache_dir)
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
...
def _load_stale_index_cache(cache_dir: Optional[Path] = None) -> Optional[dict]:
...
class HermesIndexSource(SkillSource):
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_hermes_index(cache_dir=self._cache_dir)
            self._loaded = True
        return self._index or {}

    def _get_github(self) -> GitHubSource:
        if self._github is None:
            self._github = GitHubSource(auth=self.auth, cache_dir=self._cache_dir)
        return self._github
...
def create_source_router(auth: Optional[GitHubAuth] = None,
                         cache_dir: Optional[Path] = None,
                         taps_path: Optional[Path] = None) -> List[SkillSource]:
    """
    Create all configured source adapters.
    Returns a list of active sources for search/fetch operations.

    Args:
        auth: GitHubAuth instance. Created automatically if None.
        cache_dir: Directory for index cache. If None, uses default.
        taps_path: Path to taps.json. If None, uses default.
    """
    if auth is None:
        auth = GitHubAuth()

    taps_mgr = TapsManager(path=taps_path)
    extra_taps = taps_mgr.list_taps()

    sources: List[SkillSource] = [
        OptionalSkillSource(),        # Official optional skills (highest priority)
        HermesIndexSource(auth=auth, cache_dir=cache_dir), # Centralized index (search + resolved install paths)
        SkillsShSource(auth=auth, cache_dir=cache_dir),
        WellKnownSkillSource(cache_dir=cache_dir),
        UrlSource(),                  # Direct HTTP(S) URL to a SKILL.md file
        GitHubSource(auth=auth, extra_taps=extra_taps, cache_dir=cache_dir),
        ClawHubSource(cache_dir=cache_dir),
        ClaudeMarketplaceSource(auth=auth, cache_dir=cache_dir),
        LobeHubSource(cache_dir=cache_dir),
    ]
